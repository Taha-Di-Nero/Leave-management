...
namespace Seac.Coverage.Repositories
{
    public class LeaveRepository : BaseRepository<Leave>, ILeaveRepository
    {
        ...
        public IEnumerable<Leave> GetLeavesRange(DateTime init, DateTime end) =>
            Context.Set<Leave>().AsNoTracking().Include(l => l.Employe.EmployeArea).ThenInclude(ea => ea.Area).Include(l => l.Employe.EmployeArea).ThenInclude(ea => ea.Employe).Where(l => l.Date >= init && l.Date <= end).OrderBy(l => l.Date).ToList();
